...
variable {X : Type*} {s t : TopologicalSpace X}

/-- Suppose we have two Hausdorff topologies `s, t` on `X`. If `t` is finer than `s` and `X` is
compact under `t`, then `s = t`. -/
theorem _root_.CompactSpace.eq_of_le_compact_t2 {s t : TopologicalSpace X}
    (hst : t ≤ s) (hs : @T2Space X s) (ht : @CompactSpace X t) : s = t := by
...
theorem _root_.CompactSpace.nonCompact_of_lt_compact_t2 {s t : TopologicalSpace X}
    (_ : @CompactSpace X s) (hst : t < s) (hs : @T2Space X s) : @NoncompactSpace X t := by
  refine not_compactSpace_iff.1 (fun hct => ?_)
  grind [hct.eq_of_le_compact_t2 hst.le hs]

/-- Suppose we have two Hausdorff topologies `s, t` on `X`. If `t` is finer than `s` and `B` is
a compact subset of `X` under `t`, then the subspace topology on `B` induced by `s` is equal to the
one induced by `t`. -/
theorem _root_.IsCompact.eq_of_le {s t : TopologicalSpace X} (hst : t ≤ s) {B : Set X}
    (hs : @T2Space X s) (hB : @IsCompact X t B) :
