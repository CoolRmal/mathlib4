...
theorem conditional_jensen [IsFiniteMeasure μ] (hm : m ≤ mα)
...
/-- Conditional Jensen's inequality of positively homogeneous functions for
hereditarily Lindelof Spaces. -/
private lemma conditional_jensen_of_hereditarilyLindelofSpace' [HereditarilyLindelofSpace E]
    (_ : m ≤ mα) (hφ_cvx : ConvexOn ℝ Set.univ φ) (hφ_cont : LowerSemicontinuous φ)
    (hph : ∀ x, ∀ᶠ μ in atTop, φ (μ • x) = μ * φ x) (hf_int : Integrable f μ)
    (hφ_int : Integrable (φ ∘ f) μ) :
    ∀ᵐ a ∂μ, φ (μ[f | m] a) ≤ μ[φ ∘ f | m] a := by
  obtain ⟨L, hL⟩ := hφ_cvx.real_univ_sSup_of_nat_linear_eq hφ_cont hph
  have hp : ∀ᵐ a ∂μ, ∀ i : ℕ, μ[(L i) ∘ f | m] a = (L i) (μ[f | m] a) :=
    ae_all_iff.2 fun i => ((L i).comp_condExp_comm hf_int).symm
  have hw : ∀ᵐ a ∂μ, ∀ i : ℕ, μ[(L i) ∘ f | m] a ≤ μ[φ ∘ f | m] a := by
    refine ae_all_iff.2 fun i => condExp_mono ((L i).integrable_comp hf_int) hφ_int ?_
    filter_upwards with a
    simp only [comp_apply, ← congrFun hL.2 (f a), iSup_apply]
    exact le_ciSup (bddAbove_def.2 ⟨φ (f a), fun r ⟨z, hz⟩ => hz ▸ hL.1 z (f a)⟩) i
  filter_upwards [hp, hw] with a hp hw
  simpa [← hL.2, iSup_congr hp] using ciSup_le hw

/-- **Conditional Jensen's inequality**: in a Banach space `X` with a measure `μ`, if `φ : X → ℝ` is
a convex, lower-semicontinuous, and positively homogeneous function, then for any `f : α → X` such
that `f` and `φ ∘ f` are integrable, we have `φ (𝔼[f | m]) ≤ 𝔼[φ ∘ f | m]`. -/
theorem conditional_jensen' (hm : m ≤ mα)
  ...
  calc
    φ ∘ μ[f | m]
      =ᵐ[μ] φY ∘ μ[fY | m] := by filter_upwards [lem2] with a ha; simp [φY, ha]
    _ ≤ᵐ[μ] μ[φY ∘ fY | m] := conditional_jensen_of_hereditarilyLindelofSpace' hm
      (hφ_cvx.comp_linearMap Y.subtype) (hφ_cont.comp Y.subtypeL.cont) (fun y => hph y) hfY_int
      (hφ_int.congr lem3)
    _ =ᵐ[μ] μ[φ ∘ f | m] := condExp_congr_ae lem3.symm

/-- **Conditional Jensen's inequality**: in a Banach space `X` with a measure `μ`, if `φ : X → ℝ` is
a convex, lower-semicontinuous, and positively homogeneous function, then for any
`f : α → X` such that `f` is integrable, we have `‖𝔼[f | m])‖ ≤ 𝔼[‖f‖ | m]`. -/
theorem conditional_jensen_norm (hm : m ≤ mα) (hf_int : Integrable f μ) :
    norm ∘ μ[f | m] ≤ᵐ[μ] μ[norm ∘ f | m] := by
  refine conditional_jensen' hm convexOn_univ_norm continuous_norm.lowerSemicontinuous
    (fun x => ?_) hf_int hf_int.norm
  filter_upwards [Ici_mem_atTop 0] with r hr
  simp [norm_smul_of_nonneg hr]
...
`φ ∘ f` are integrable, we have `φ (𝔼[f | m]) ≤ 𝔼[φ ∘ f | m]`. -/
theorem conditional_jensen_finite_dim [IsFiniteMeasure μ] (hm : m ≤ mα)
