...
/-- A kernel in the image of a set of measures, seen as an element of the full range of
`of_measure`. -/
def imageToRangeOfMeasure (θ Ω : Type*) [MeasurableSpace θ]
    [mΩ : MeasurableSpace Ω] (s : Set (Measure Ω)) :
    (of_measure θ Ω) '' s → Set.range (of_measure θ Ω) :=
  Set.inclusion (Set.image_subset_range (of_measure θ Ω) s)

theorem measurable_imageToRangeOfMeasure (θ Ω : Type*)
    [MeasurableSpace θ] [mΩ : MeasurableSpace Ω] {s : Set (Measure Ω)} :
    Measurable (imageToRangeOfMeasure θ Ω s) :=
